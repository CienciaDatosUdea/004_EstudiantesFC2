...
Grid::Grid(double x_min_, double x_max_, double y_min_,
    double y_max_, int nx_, int ny_)
    :x_min(x_min_), x_max(x_max_), y_min(y_min_), y_max(y_max_), nx(nx_), ny(ny_){
        
    dx = (x_max-x_min)/(nx-1);
    dy = (y_max-y_min)/(ny-1);
    XY.resize(nx, vector<vector<double>>(ny,vector<double>(2)));
...
void CampoVelocidadesPresion::CondicionesIniciales(){
    U.resize(G.nx, vector<double>(G.ny, 1E-10)); 
    V.resize(G.nx, vector<double>(G.ny, 1E-10));
    P.resize(G.nx, vector<double>(G.ny, 10)); 
}
...
void CampoVelocidadesPresion::BoundaryCondition(){
    ...
    for(int i=0; i<G.nx; i++){
        ...
        // P[i][G.ny-1] = 0; // Constante de integración 
    }
}

ComputePresion::ComputePresion(CampoVelocidadesPresion &Campo_,
    double rho_, double dt_, int nit_)
...
void ComputePresion::presionUpdate(){
    ...
    vector<vector<double>> Pn = Campo.P;
...
ComputeCampo::ComputeCampo(ComputePresion &omega_, double F_):omega(omega_), F(F_){}
...
void ComputeCampo::velocidadesUpdate(){
    ...
    vector<vector<double>> Un = omega.Campo.U;
    vector<vector<double>> Vn = omega.Campo.V;
    vector<vector<double>> Pn = omega.Campo.P;
    ...
    for(int i=1; i < nx-1; i++){
        for(int j=1; j < ny-1; j++){
            ...
            - dt / (2.0 * rho *dx) * (Pn[i+1][j] - Pn[i-1][j])
            + nu*dt * (
            ...
            omega.Campo.V[i][j] = Vn[i][j]
                ...
                - dt / (2.0 * rho * dy) * (Pn[i][j + 1] - Pn[i][j - 1])
                + nu * dt * (
...
FieldWriter::FieldWriter(const std::string &filename)    
    {
        ...
        file << fixed;// << precision(6);
    }

void FieldWriter::writePosicionCampo(const Grid &G, 
