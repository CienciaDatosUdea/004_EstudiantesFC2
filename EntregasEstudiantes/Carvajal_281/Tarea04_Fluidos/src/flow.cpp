/* Para esta tarea se toma como punto de partida el código trabajado en clase, por lo que explicaré un poco lo que ya está implementado agregando una fuerza externa al campo de velocidades y modificando las condiciones de frontera para evitar sobrerestricción*/
...
void CampoVelocidadesPresion::BoundaryCondition(){   
    ...
    for(int i=0;i<G.nx;i++){
        U[i][0] = 0;
        V[i][0] = 0; //Eliminamos la condición de neumann homogenera para la presión, asi evitamos sobre restricciones
    }
    //Frontera Izquierda: Condiciones de pared rígida, no hay flujo a través de la frontera
...
void ComputeCampo::velocidadesUpdate() {
    ...
    double nu = 0.1;
    double Fx = 0.2; // Agregamos el valor de la fuerza externa en x 
    double Fy = 0.4; // Agregamos el valor de la fuerza externa en y
    ...
    for (int i = 1; i < nx - 1; i++) {
        for (int j = 1; j < ny - 1; j++) {
            // Aqui se actualiza la componente U agregando una fuerza externa constante
