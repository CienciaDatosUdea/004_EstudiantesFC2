...
// Immediately initializes spins and sweeps the system in the constructor body. This way, constructing the object is a one-and-done operation and the results can immediately be used, ideal for our purposes.
...
// returns a random real number from [0,1), used for the monte carlo simulation
double Ising::IsingUniform() const {
    static std::uniform_real_distribution<double> dist(0.0, 1.0);
    return dist(GlobalRng());
}

// outputs an int from [0, L_-1] randomly, used to choose a random index
int Ising::RandomIndex() const {
    static std::uniform_int_distribution<int> dist(0, L_-1);
    return dist(GlobalRng());
}

// Initializes all spins by individually assigning -1 or 1 randomly
...
// calculates the sum of the four 2D neighbors of a spin
...
// calculates the local energy based on the spin, its next neighbor sum and J
...
// returns vector holding spin configuration
...
// calculates and returns the total magnetization
